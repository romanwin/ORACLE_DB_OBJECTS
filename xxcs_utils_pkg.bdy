CREATE OR REPLACE PACKAGE BODY xxcs_utils_pkg IS
  ...
  BEGIN
    ...
    BEGIN
      SELECT MAX(tt.task_id)
        INTO v_task_id
        FROM jtf_tasks_b tt, jtf_task_statuses_b stt
  ...
  FUNCTION get_sr_related_orders_message(p_incident_id NUMBER)
    ...
    CURSOR get_sr_related_orders IS
      SELECT 1, charges_order_tab.line_text
      ...
      UNION
      SELECT 2, repair_sales_order_tab.line_text
      ...
      UNION
      SELECT 3, parts_requirment_order_tab.line_text
      ...
      UNION
      SELECT 4, move_order_tab.line_text
        FROM (SELECT 'Move order #' || mh.request_number || ' ' || 'is open' line_text
                FROM mtl_txn_request_headers mh, cs_incidents_all_b cb
      ...
      UNION
      SELECT 5, open_tasks_tab.line_text
               ...
               WHERE cb.incident_id = p_incident_id --param
                 AND cb.incident_id NOT IN
                     (SELECT t.source_object_id
                        FROM jtf_tasks_b t, jtf_task_statuses_b s
      ...
      UNION
      SELECT 7, 'Resolution code is missing' text
        FROM cs_incidents_all_b ciab
       WHERE ciab.incident_id = p_incident_id --param
  ...
  BEGIN
    ...
    FOR orders_info_rec IN get_sr_related_orders LOOP
  ...
  BEGIN
    SELECT rs_reg_mang.resource_name
      ...
      FROM jtf_rs_resource_extns_vl rs_reg_mang, jtf.jtf_rs_salesreps t
  ...
  BEGIN
    ...
    FOR rec IN (SELECT to_char(cii.attribute8) region,
                 ...
                 WHERE cii.attribute8 IS NOT NULL
                   ...
                   AND cii.instance_id = ciab.customer_product_id) LOOP
      BEGIN
  ...
  BEGIN
    ...
    --, cia.incident_id, cia.incident_number, cia.creation_date
      INTO l_reactive, l_customer_product_id, l_incident_occurred_date
      FROM cs_incident_types_b cit, cs_incidents_all_b cia
  ...
  PROCEDURE update_tm_pl_in_ib(errbuf OUT VARCHAR2, retcode OUT VARCHAR2) IS
  
    CURSOR ib_instance IS
      SELECT cii.instance_id, rule_pl rule_pl_id
  ...
  BEGIN
    ...
    IF nvl(fnd_profile.value('XXCS_OM_CHK_TECH_ENTITLE'), 'Y') = 'Y' THEN
      ...
      SELECT xxinv_utils_pkg.get_category_segment('SEGMENT6',
        ...
        INTO v_pr_technology
        FROM csi_item_instances cii, mtl_system_items_b msib
    ...
    IF p_instance_id IS NOT NULL THEN
      FOR l_entitlement_type_rec IN c_entitlement_type(p_instance_id) LOOP
    ...
    CURSOR upd_printer_instance IS
      SELECT DISTINCT oola_upg.line_id upg_line_id,
        ...
        FROM oe_order_lines_all oola_upg,
             ...
             (SELECT upgrade_item_id, before_upgrade_item
                FROM xxcs_sales_ug_items_v upg
              UNION ALL
              SELECT msi.inventory_item_id, flv.attribute1
                FROM fnd_lookup_values flv, mtl_system_items_b msi
  ...
  BEGIN
    ...
    FOR upg_so_line_rec IN upd_printer_instance LOOP
  ...
  ---------------------------------------------------------------------------------------
  function get_item_number_by_serial(p_instance_number VARCHAR2)
    return varchar2 is
    -------------------------
    --   Local Definition
    -------------------------
